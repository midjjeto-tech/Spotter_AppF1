...
#
# ТЕМБРА ДВА, А НЕ ЧЕТЫРЕ, И ЭТО ЛИЦЕНЗИОННОЕ ОГРАНИЧЕНИЕ, А НЕ ВКУСОВОЕ.
# ru_RU-ruslan-medium раздаётся под CC BY-NC-SA 4.0 (корпус RUSLAN), у
# ru_RU-irina-medium условия не установлены вовсе — оба непригодны для
# коммерческого распространения и удалены из models/piper/ (см. NOTICE, аудит
# 2026-08-08). Остались denis и dmitri — оба CC0. Возвращать удалённые модели
# нельзя; на это стоит гейт в build.ps1.
#
# Раскладка по приоритету КАНАЛОВ, а не персон. Инженер и споттер говорят в
# одну гонку, и спутать их на слух — потерять смысл предупреждения, поэтому
# различие между ними обязательно. Персоны комментатора (tv/hype/calm/toxic)
# на двух тембрах развести уже нечем — они отличаются только темпом. Это
# сознательная потеря: качество голоса в проекте держится на Yandex, а Piper —
# аварийный фолбэк на случай отказа сети.
PERSONA_VOICE: dict[str, tuple[str, float]] = {
    "tv":    ("denis",  1.0),
    "hype":  ("denis",  0.92),
    "calm":  ("dmitri", 1.08),
    "toxic": ("dmitri", 1.0),
    # Слоты ролей (см. yandex_ai/voices.py). Динамически уступить голос, как это
    # делает voice_cast.resolve(), путь Piper не может: PERSONA_VOICE.get(persona)
    # статичен, механизма оверрайдов у него нет. Совпадение роли с персоной
    # комментатора возможно и допустимо; совпадение инженера со споттером — нет.
    "engineer": ("dmitri", 1.0),
    "spotter":  ("denis",  1.12),
}
_DEFAULT_VOICE = "denis"
