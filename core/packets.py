...
#
# COLL здесь БОЛЬШЕ НЕТ (2026-08-11). Игра шлёт COLL на любое касание — от
# притирки боком до настоящей аварии, — а тяжести в самом пакете нет вовсе
# (см. core/radio/policy.py: «COLL знает только, что контакт был»). Безусловный
# critical означал важность 90+ и директиву LLM «авария» на КАЖДОЕ касание,
# отсюда «ХАОС НА ТРАССЕ» по десять раз за гонку. Тяжесть теперь выводится из
# последствия — см. core/engine.py::_grade_contact, который ждёт данных о
# повреждении и публикует COLL_LIGHT / COLL / COLL_HEAVY.
CRITICAL_EVENTS = {"PENA", "RTMT", "CHQF", "RCWN", "SCAR", "RDFL"}
...
def _lap_delta_ms(data: bytes, base: int, ms_off: int, min_off: int) -> int | None:
    """Дельта F1 25 формата «минуты + мс» -> суммарные мс (как у секторов).

    None = дельты нет. `ms_part` по построению формата — остаток МЕНЬШЕ минуты
    (целые минуты лежат отдельным байтом), поэтому значение >= 60000 не может
    быть дельтой: это заглушка игры, когда отрыв не определён. Порог не
    подобран под наблюдения, а следует из самого формата.

    Найдено разбором живого заезда 2026-08-11: девять значений в диапазоне
    65053-65520 мс уезжали в стратега как настоящие отрывы, и он принимал на
    них решения («push, ers_overtake, gap_behind=65520»). Живые значения там —
    сотни и тысячи мс, так что молча пропущенный мусор выглядел как отрыв в
    65 секунд у машины, идущей в полутора секундах.
    """
