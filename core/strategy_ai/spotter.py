...
# Анти-дребезг на границе порога. Откалибровано по разбору живого заезда
# 2026-08-11: при 3.0 споттер дал 32 боковых предупреждения за 5 минут
# квалификации — три четверти всего эфира, и большая часть была оборвана
# следующим предупреждением. Порог обнаружения (LATERAL_*) при этом НЕ трогали:
# споттер остаётся безопасностью, реже он предупреждает об одном и том же, а не
# позже о новом.
MIN_REPEAT_S = 6.0
