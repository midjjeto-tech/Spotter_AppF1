...
#: правда, а не поломка.
#:
#: Владелец числа — `slip.py`: с 2026-08-25 тот же порог отсекает старт с
#: решётки в самом детекторе, и два независимых числа в двух модулях однажды
#: разошлись бы молча.
MIN_MOVING_KMH = slip_mod.MIN_MOVING_KMH
...
SILENCE_RU: dict[str, str] = {
    ...
    "signal_not_trusted":
        "Сигнал о срывах не сходится с физикой — подсказки придержаны, "
        "чтобы не назвать не то колесо.",
    "signal_warming_up":
        "Коуч ещё набирает данные и молчит, пока не убедится в сигнале.",
}
...
class CoachHealth:
    ...
    def trusted(self) -> bool:
        """Можно ли ГОВОРИТЬ по этому сигналу.

        Вердикт `signal` считался с самого начала, показывался на экране — и ни
        на что не влиял: детектор публиковал подсказки независимо от того, что
        про сигнал думает эта же сессия. То есть модуль, заведённый ответить
        «почему коуч молчит», не умел заставить его замолчать.

        Разбор архива 2026-08-15 показал цену. В реальных заездах Майами пики
        `wheelspin` достигали 5.4 при `SANE_MAX_SLIP_RATIO = 3.0` — то есть
        приложение СВОИМИ ЖЕ порогами объявляло данные невозможными и всё равно
        по ним советовало. Ровно тот случай, ради которого коуч и держат
        выключенным: «назовёт не то колесо, и пилот будет чинить не то».

        Строго `ok`: `warming_up` тоже молчит. Это первые ~10 секунд движения,
        цена нулевая, а правило простое — не говорить, пока не убедились, что
        сигнал настоящий."""
        return self.signal == SIGNAL_OK

    @property
    def silence_reason_for_signal(self) -> str:
        """Причина молчания для журнала и экрана — под ключи `SILENCE_RU`."""
        return ("signal_warming_up" if self.signal == SIGNAL_WARMING_UP
                else "signal_not_trusted")
