...
DEFAULTS: dict = {
    ...
    # разные потребности. Выключено по умолчанию, пока пороги детекторов
    # (core/coach_ai/slip.py) не откалиброваны на живой сессии — коуч, который
    # может назвать не то колесо, хуже выключенного.
    "driving_coach_enabled":    False,
    # Приглушать звук игры на время реплики — то, что в настоящей F1 делает
    # радиоканал. Выключено по умолчанию: обновление не должно молча убавлять
    # пользователю звук игры (тот же принцип, что у тем оверлея).
    # `game_ducking_level` — до скольких процентов от ТЕКУЩЕЙ громкости игры
    # приглушаем; доля, а не абсолютная величина, иначе у того, кто уже играет
    # тихо, «приглушение» сделало бы громче.
    "game_ducking_enabled":     False,
    "game_ducking_level":       35,
    # Полевой журнал заезда (core/field_log.py) — подробная диагностика в
    # DATA_DIR/field-diag-*.jsonl: распределения сигналов коуча, причины его
    # молчания, что нашёл микшер. Нужен, чтобы разбирать живой заезд ПОСЛЕ него,
    # в том числе заезд, в котором ничего не сработало. Выключен по умолчанию:
    # обычному пользователю он не нужен, а файл растёт. В дереве разработки то
    # же самое включает переменная окружения SPOTTER_DIAG=1.
    "field_diagnostics":        False,
    # Второй экран: открыть UI по локальной сети, чтобы держать таймингборд и
    # ленту на телефоне рядом с рулём. Выключено по умолчанию — через это же
    # API меняются настройки и записываются ключи Yandex/GigaChat, поэтому порт
    # наружу не открывается сам. Токен генерируется при включении, а не лежит в
    # дефолтах: одинаковый токен у всех сборок это отсутствие токена.
    # Политика доступа — core/remote_access.py.
