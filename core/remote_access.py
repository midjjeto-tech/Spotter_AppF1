...
#: Кука, которой браузер носит токен дальше сам.
#:
#: Без неё фича не работала вообще. Токен ехал двумя путями — один раз в адресе
...
#: Гейт стоит перед всей раздачей, поэтому телефон получал документ (200) и 401
#: на каждый его подресурс — голый HTML без стилей и без гидратации React.
#: Кука прикладывается браузером ко всему, включая перезагрузку документа по
#: «чистому» адресу (токен из адресной строки вычищается после загрузки).
COOKIE_NAME = "spotter_remote_token"

#: 30 дней: срок жизни закладки на телефоне, а не сессии. Пересоздание токена
#: делает старую куку недействительной само по себе — сверка идёт с настройками.
...
class RemoteAccessPolicy:
    ...
    def allows(self, client_addr: str | None, path: str, method: str,
        ...
        if path in LOCAL_ONLY_PATHS:
            ...
            return False
        return self.accepts_token(token)

    def accepts_token(self, token: str | None) -> bool:
        """Токен верен сам по себе — без учёта адреса клиента и пути.

        Отдельно от `allows`, потому что решение «выдать куку» принимается
        именно по предъявленному токену: `allows` пропускает локального клиента
        и без токена, а `LOCAL_ONLY_PATHS` не пускает и с верным."""
        if not self.enabled or not self.token:
            return False
