...
# ── Политика относительно уже звучащей фразы — УДАЛЕНА 2026-08-14 ────────────
#
# Здесь жили четыре константы (`interrupt` / `next` / `replace` / `drop_if_busy`)
# и `interrupt_policy_for()`, считавшая политику для КАЖДОГО сообщения. Поле
# доезжало до `RadioMessage.interrupt_policy` и оттуда — ровно в одну строку
# `_log.debug`. Никто его не читал.
#
# Убрано не за неиспользуемость, а потому что оно ВРАЛО. Реально прерывание
# решают два механизма в `new_tts/queue_handler.py`: ранг (из urgency) и
# `_NO_SELF_INTERRUPT` (по категории). Из четырёх политик очередь реализует
# полторы: critical действительно прерывает, high/normal встают в очередь по
# рангу, а `replace` и `drop_if_busy` не делает НИКТО. То есть в логе стояло
# `policy=drop_if_busy` у реплики, которая спокойно ждала своей очереди за тремя
# другими. Диагностика, показывающая не то, что происходит, хуже её отсутствия —
# на ней и разбирают проды.
#
# **Замысел сохранён здесь намеренно, чтобы не изобретать заново.** Если такая
# политика когда-нибудь понадобится всерьёз, недостающие два случая означают:
#   `replace`       — вытеснить ОЖИДАЮЩЕЕ сообщение той же категории новым
#                     (сейчас частично закрыто дедупом ситуаций выше уровнем);
#   `drop_if_busy`  — не ставить в очередь вовсе, если радио занято; для низкой
#                     срочности это согласуется с принципом проекта «молчание
#                     дешевле опоздавшей реплики».
# Начинать надо с реализации в очереди, а не с восстановления этого поля:
# поле без потребителя — ровно то, что здесь и лежало.
...
# Категории, где новое сообщение ЛОГИЧЕСКИ вытесняет прежнее: периодические
# сводки о медленно меняющемся состоянии. Две сводки о гэпе подряд — это не
# диалог, это устаревшая и свежая версия одного факта.
#
# Список сохранён как ЗНАНИЕ О ПРЕДМЕТЕ, а не как рабочая таблица: его
# единственный потребитель (`interrupt_policy_for`) удалён 2026-08-14 вместе со
# всей мёртвой политикой — см. комментарий в шапке. Сегодня повтор сводок
# закрывается уровнем выше, дедупом ситуаций (`core/situation_dedup.py`,
# `EngineerTopicDedup`). Если очередь однажды научится вытеснению, начинать
# стоит отсюда.
REPLACEABLE_CATEGORIES: frozenset[str] = frozenset({
