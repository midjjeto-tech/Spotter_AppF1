...
class Commentator:
    ...
    def create_broadcast(self, event: dict, ai_ok: bool = True) -> str:
        ...
        При любом отказе падает на единый банк формулировок
        (`commentator/radio.py::race_ai_phrase` -> `core/radio/phrases.py`), а
        не на отдельный набор шаблонов: манера речи не должна меняться от того,
        сработал LLM или нет.
        """
        ...
        phrase = _radio.race_ai_phrase(
            event.get("race_ai_type", "stable"), race_ai_data,
            selector_key=event.get("dedupe_key") or f"broadcast:{code}",
        )
        if phrase:
